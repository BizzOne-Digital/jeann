...
export function LoginForm() {
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setMessage(result.body.error || "Unable to sign in. Check email and password, or contact support.");
  }
  return (
...
export function BuyerRegistrationForm() {
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const result = await submit("/api/auth/register/buyer", {
      ...values,
      acceptBuyerTerms: form.get("terms") === "on",
    ...
    setMessage(
      result.body.message ||
        (result.ok
          ...
          : result.body.error || "Unable to register. Password must be at least 12 characters."),
    );
  }
  return (
